...
const mySqlMongooseSchemaFormatter = require('./mysql/formatter/mongoose-schema');
...
class Parser {
  ...
  constructor(dialect = 'mysql') {
    if (!dialect || dialect === 'mysql' || dialect === 'mariadb') {
      ...
      this.mongooseSchemaFormatter = mySqlMongooseSchemaFormatter;
    }
    else {
   ...
   * 
   * @param {any[]} tables Array of tables in compact JSON format (optional).
   * @returns {any[]} Mongoose Schema documents array.
   */
  toMongooseSchemaArray(tables = null) {
    if (!tables) {
      tables = this.toCompactJson();
    }

    return this.mongooseSchemaFormatter.format(tables);
  }
   ...
   * @param {string} outputDir Output directory.
   * @param {JsonSchemaOutputOptions} options Options object for JSON Schema output to files (optional).
   * @param {any[]} mongooseSchemas Mongoose Schema documents array (optional).
   * @returns {Promise<string[]>} Resolved promise with output file paths.
   */
  toMongooseSchemaFiles(outputDir, options = {}, mongooseSchemas = null) {
    if (!outputDir) {
      throw new Error('Please provide output directory for JSON Schema files');
    }
    ...
    if (!mongooseSchemas) {
      mongooseSchemas = this.toMongooseSchemaArray();
    }
    ...
    return Promise.all(
      mongooseSchemas.map(schema => {
        return new Promise(resolve => {
          if (!schema.title) {
            throw new Error('No title found in schema. It should contain the table name. ' +
              'If you have modified the JSON Schema, please keep the title, as it will be the file name.');
          }
          const filename = schema.title;
          const filepath = join(outputDir, filename + options.extension);


          const str = JSON.stringify(schema.columns, function (key, val) {
          ...
          fs.writeFile(filepath, 'module.exports = '+ str.replace(/\"/g, '') + ';', err => {
            if (err) {
              throw new Error(`Error when trying to write to file ${filepath}: ${JSON.stringify(err, null, 2)}`);
            }
            resolve(filepath);
